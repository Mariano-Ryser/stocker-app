...
export default function UserDetailModal({ 
  ...
  const { fetchProductLimits, clearProductsFromCache, clearCompanyCache } = useProduct();
  ...
  useEffect(() => {
    ...
    if (userData?.company?.maxProducts) {
      setNewProductLimit(userData.company.maxProducts);
    }
  }, [userData]);
  ...
  useEffect(() => {
    if (userData?._id && userData?.companyId) {
      ...
      // ✅ Cargar límites de la empresa del usuario seleccionado
      const loadCompanyLimits = async () => {
        setLimitsLoading(true);
        try {
          const limits = await fetchProductLimits(userData.companyId);
          if (limits) {
            setCompanyLimits(limits);
            setNewProductLimit(limits.max);
          }
        } catch (err) {
          console.error('Error loading company limits:', err);
        } finally {
  ...
  }, [userData?._id, userData?.companyId, fetchUserDetails, fetchProductLimits, clearUserDetails]);
  ...
  const handlePlanChange = async (newPlan) => {
    ...
    try {
      ...
      // ✅ Recargar límites después de cambiar plan
      const limits = await fetchProductLimits(userData.companyId);
      if (limits) {
        setCompanyLimits(limits);
        setNewProductLimit(limits.max);
  ...
  const handleProductLimitChange = async () => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        // ✅ Actualizar companyLimits
        setCompanyLimits(prev => ({
          ...prev,
          max: newProductLimit,
          remaining: Math.max(0, newProductLimit - (prev?.current || 0))
        }));
  ...
  const handleDeleteCompany = async () => {
    ...
    // ✅ OBTENER TODOS LOS IDs DE PRODUCTOS ANTES DE ELIMINAR
    const allProductIds = userDetails?.products?.map(p => p._id) || [];
    ...
    if (result.success) {
      ...
      // ✅ LIMPIAR CACHÉ COMPLETO DE LA EMPRESA
      if (typeof window !== 'undefined') {
        if (deleteOptions.products && allProductIds.length > 0) {
          window.dispatchEvent(new CustomEvent('clearProductsFromCache', {
            detail: { productIds: allProductIds }
          }));
        }
        
        window.dispatchEvent(new CustomEvent('clearCompanyCache', {
          detail: { companyId: userData.companyId }
        }));
      }
  ...
  const handlePartialDelete = async (type) => {
    ...
    switch(type) {
      case 'products':
        // ✅ OBTENER IDs DE PRODUCTOS ANTES DE ELIMINAR
        if (userDetails?.products && userDetails.products.length > 0) {
          deletedProductIds = userDetails.products.map(p => p._id);
        }
        
        result = await cleanupProducts(userData.companyId);
        message = 'productos';
        
        // ✅ LIMPIAR CACHÉ DE PRODUCTOS ELIMINADOS
        if (deletedProductIds.length > 0 && typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('clearProductsFromCache', {
            detail: { productIds: deletedProductIds }
          }));
        }
        break;
    ...
    if (result.success) {
      ...
      await fetchUserDetails(userData._id, userData.companyId);
      // ✅ Recargar límites después de eliminar productos
      const limits = await fetchProductLimits(userData.companyId);
      if (limits) {
        setCompanyLimits(limits);
      }
      
      // ✅ SI SE ELIMINARON PRODUCTOS, FORZAR RECARGA DEL CACHÉ
      if (type === 'products') {
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('refreshScannerProducts'));
        }
      }
    } else {
  ...
  // ✅ USAR companyLimits en lugar de productLimits
  const currentMaxProducts = companyLimits?.max || localUserData?.company?.maxProducts || 100;
  const currentProductsCount = stats.products;
  
  const isLoading = loading || cleanupLoading || actionLoading || changingPlan || changingProductLimit || limitsLoading;
              ...
              <div className={styles.companyInfo}>
                ...
                <div className={styles.companyStats}>
                  ...
                  <span className={styles.companyStat}>
                    ...
                    <span className={styles.statValue}>
                      {limitsLoading ? '...' : currentMaxProducts}
                    </span>
                  </span>
                </div>
              </div>
              ...
              <div className={styles.limitInfo}>
                <div className={styles.limitCurrent}>
                  ...
                  <span className={styles.limitValue}>
                    {limitsLoading ? '...' : `${currentProductsCount} / ${currentMaxProducts}`}
                  </span>
                  <span className={styles.limitPercentage}>
                    ({limitsLoading ? '...' : Math.round((currentProductsCount / currentMaxProducts) * 100)}%)
                  </span>
                </div>
                ...
                <div className={styles.limitProgressBar}>
                  <div 
                    ...
                    style={{ 
                      width: `${Math.min(100, (currentProductsCount / currentMaxProducts) * 100)}%`,
                      backgroundColor: (currentProductsCount / currentMaxProducts) >= 0.9 ? '#ef4444' : 
                                     (currentProductsCount / currentMaxProducts) >= 0.75 ? '#f59e0b' : '#10b981'
                    }}
                  />
                </div>
                ...
                <p className={styles.limitNote}>
                  ...
                  {newProductLimit < currentProductsCount && (
